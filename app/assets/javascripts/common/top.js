...
$(document).ready(function() {
  ...
  function setFont(message, x, y, r, g, b, alpha) {
    ...
    context.fillText(message, width * .5 - context.measureText(message).width/2 + x, y);
  }
  ...
  (function draw() {
    ...
    context.stroke();
    */
    context.font = "700 " + height * .1 + "px 'Rounded Mplus 1c', '游ゴシック'";
    ...
    context.font = "700 " + (height * (.06 + animeCount[8] * .18)) + "px 'Rounded Mplus 1c'";
    
    ratio = .23;
    setFont("imagination", 0, height * (ratio + ((.55 - ratio) * animeCount[8])), 177, 233, 255, animeCount[4] - animeCount[9]);
    ratio += .25;
    setFont("association", 0, height * (ratio + ((.55 - ratio) * animeCount[8])), 177, 233, 255, animeCount[5] - animeCount[9]);
    ratio += .25;
    setFont("connection" , 0, height * (ratio + ((.55 - ratio) * animeCount[8])), 177, 233, 255, animeCount[6] - animeCount[9]);
    
    //context.font = "700 " + height * .2 + "px 'Rounded Mplus 1c'";
    //setFont("Imaginnection", 0, height * .5, 177, 233, 255, animeCount[10]);
    
    context.font = "700 " + height * .07 + "px 'Rounded Mplus 1c'";
    setFont("い　ま　じ　ね　く　し　ょ　ん", 0, height * .2, 177, 233, 255, animeCount[11]);
    
    context.font = "700 " + height * .06 + "px 'Rounded Mplus 1c'";
    setFont("イメージを 連ねて 繋がろう", 0, height * .8, 177, 233, 255, animeCount[11]);
    
    if(animeCount[10] > 0) {
      context.globalAlpha = animeCount[10];
      context.drawImage(img, Math.round(width * .5 - width/2), Math.round(height * .25), Math.round(width), Math.round(width * .24));
    } else {
    ...
    for(var i = 0; i < animeCount.length; i++) {
      if(animeCount[i] < 1.0) {
        animeCount[i] = Math.min(animeCount[i] + .02, 1.0);
        break;
      }
    }
